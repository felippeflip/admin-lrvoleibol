...
<div class="w-full max-w-md mx-auto pb-24">
    ...
    <div class="flex items-center gap-4 mb-6 px-1">
        ...
        <h2 class="text-xl font-extrabold text-gray-900 dark:text-white">Editar Atleta</h2>
    </div>
    ...
    <form action="{{ route('atletas.update', $atleta->atl_id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 flex flex-col items-center">
            <div class="relative group mb-6">
                <div class="w-32 h-44 rounded-2xl overflow-hidden border-2 border-dashed border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 flex items-center justify-center group-active:scale-95 transition-all">
                    <img id="fotoPreview" src="{{ ($atleta->atl_foto && \Storage::disk('atletas_fotos')->exists($atleta->atl_foto)) ? asset('storage/atletas_fotos/' . $atleta->atl_foto) : asset('images/placeholder-atleta.png') }}" class="w-full h-full object-cover">
            ...
            <div class="w-full space-y-4">
                <div>
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Reg. Liga Volley (Não Editável)</label>
                    <input type="text" value="{{ $atleta->atl_resg }}" readonly 
                           class="w-full bg-gray-50 dark:bg-gray-900 border-none rounded-2xl p-4 font-black text-sm text-gray-500 uppercase tracking-widest cursor-not-allowed">
                </div>
                <div>
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Nome Completo *</label>
                    <input type="text" name="atl_nome" value="{{ old('atl_nome', $atleta->atl_nome) }}" required 
                ...
                <div class="flex gap-4">
                    <div class="flex-1">
                        ...
                        <input type="date" name="atl_dt_nasc" value="{{ old('atl_dt_nasc', $atleta->atl_dt_nasc) }}"
                    ...
                    <div class="w-24">
                        ...
                        <select name="atl_sexo" class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-xs dark:text-white appearance-none">
                            <option value="M" {{ old('atl_sexo', $atleta->atl_sexo) == 'M' ? 'selected' : '' }}>Masc</option>
                            <option value="F" {{ old('atl_sexo', $atleta->atl_sexo) == 'F' ? 'selected' : '' }}>Fem</option>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-5">
            <div class="flex items-center justify-between ml-1">
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" id="atl_estrangeiro_mobile" name="atl_estrangeiro" value="1" {{ old('atl_estrangeiro', $atleta->atl_estrangeiro) ? 'checked' : '' }}
            ...
            <div id="div_cpf_mobile" class="{{ $atleta->atl_estrangeiro ? 'hidden' : '' }}">
                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">CPF</label>
                <input type="text" name="atl_cpf" id="atl_cpf_mobile" value="{{ old('atl_cpf', $atleta->atl_cpf) }}" maxlength="14" placeholder="000.000.000-00"
                       class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-sm dark:text-white">
            </div>

            <div id="div_passaporte_mobile" class="{{ $atleta->atl_estrangeiro ? '' : 'hidden' }}">
                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Passaporte / Doc</label>
                <input type="text" name="atl_passaporte" value="{{ old('atl_passaporte', $atleta->atl_passaporte) }}" maxlength="50"
                       class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-sm dark:text-white">
            </div>

            <div>
                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">RG</label>
                <input type="text" name="atl_rg" value="{{ old('atl_rg', $atleta->atl_rg) }}" maxlength="12"
                       class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-sm dark:text-white">
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-5">
             <div>
                ...
                <input type="email" name="atl_email" value="{{ old('atl_email', $atleta->atl_email) }}"
                       class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-sm dark:text-white">
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Celular</label>
                    <input type="text" name="atl_celular" value="{{ old('atl_celular', $atleta->atl_celular) }}" maxlength="15" placeholder="(00) 00000-0000"
                           class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-sm dark:text-white">
                </div>
                <div>
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Telefone</label>
                    <input type="text" name="atl_telefone" value="{{ old('atl_telefone', $atleta->atl_telefone) }}" maxlength="14"
                           class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-sm dark:text-white">
                </div>
            </div>
        </div>
        ...
        {{-- Seção: Endereço --}}
        <details class="group bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden" {{ ($atleta->atl_endereco || old('atl_endereco')) ? 'open' : '' }}>
            ...
            <div class="p-6 pt-0 space-y-4 border-t border-gray-50 dark:border-gray-700/50">
                <div class="pt-4">
                    ...
                    <input type="text" name="atl_cep" id="atl_cep_mobile" value="{{ old('atl_cep', $atleta->atl_cep) }}" maxlength="9" placeholder="00000-000"
                           class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-sm dark:text-white">
                </div>
                <div class="grid grid-cols-4 gap-2">
                    <div class="col-span-3">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Logradouro</label>
                        <input type="text" name="atl_endereco" id="atl_endereco_mobile" value="{{ old('atl_endereco', $atleta->atl_endereco) }}"
                               class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-sm dark:text-white">
                    </div>
                    <div class="col-span-1">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Nº</label>
                        <input type="text" name="atl_numero" value="{{ old('atl_numero', $atleta->atl_numero) }}"
                               class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-xs dark:text-white">
                    </div>
                </div>
                <div>
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Bairro</label>
                    <input type="text" name="atl_bairro" id="atl_bairro_mobile" value="{{ old('atl_bairro', $atleta->atl_bairro) }}"
                           class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-sm dark:text-white">
                </div>
                <div class="grid grid-cols-4 gap-2">
                    <div class="col-span-3">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Cidade</label>
                        <input type="text" name="atl_cidade" id="atl_cidade_mobile" value="{{ old('atl_cidade', $atleta->atl_cidade) }}"
                               class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-sm dark:text-white">
                    </div>
                    <div class="col-span-1">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">UF</label>
                        <input type="text" name="atl_estado" id="atl_estado_mobile" value="{{ old('atl_estado', $atleta->atl_estado) }}" maxlength="2"
                               class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 font-bold text-xs dark:text-white">
                    </div>
                </div>
            </div>
        ...
        <div class="bg-blue-600 rounded-[3rem] p-8 text-white shadow-xl space-y-6">
            ...
            <div>
                ...
                <select name="atl_tim_id" required class="w-full bg-white/10 border-none rounded-2xl p-4 font-bold text-sm text-white appearance-none {{ !auth()->user()->hasRole('Administrador') ? 'pointer-events-none opacity-50' : '' }}">
                    @foreach($times as $time)
                        <option value="{{ $time->tim_id }}" {{ old('atl_tim_id', $atleta->atl_tim_id) == $time->tim_id ? 'selected' : '' }} class="text-gray-900">{{ $time->tim_nome }}</option>
                    @endforeach
                </select>
                @if(!auth()->user()->hasRole('Administrador'))
                    <p class="text-[9px] font-bold text-white/50 mt-2 ml-1 uppercase">Procure um Admin para transferências.</p>
                @endif
            </div>
            @endif
            ...
            <div>
                ...
                <select name="atl_categoria" required class="w-full bg-white/10 border-none rounded-2xl p-4 font-bold text-sm text-white appearance-none">
                    @foreach($categorias as $categoria)
                        <option value="{{ $categoria->cto_id }}" {{ old('atl_categoria', $atleta->atl_categoria) == $categoria->cto_id ? 'selected' : '' }} class="text-gray-900">{{ $categoria->cto_nome }}</option>
                    @endforeach
                </select>
            </div>
                ...
                <div>
                    ...
                    <select name="atl_ano_insc" class="w-full bg-white/10 border-none rounded-2xl p-4 font-bold text-sm text-white appearance-none">
                        ...
                        @foreach($years as $year)
                            <option value="{{ $year }}" {{ old('atl_ano_insc', $atleta->atl_ano_insc) == $year ? 'selected' : '' }} class="text-gray-900">{{ $year }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="pb-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        @php
                            $cartaoImpresso = \App\Models\AtletaCartao::where('atc_atl_id', $atleta->atl_id)->where('atc_ano', date('Y'))->value('atc_impresso');
                        @endphp
                        <input type="checkbox" name="cartao_impresso_ano_atual" value="1" {{ $cartaoImpresso ? 'checked' : '' }} {{ !auth()->user()->hasRole('Administrador') ? 'disabled' : '' }}
        ...
        <div class="bg-gray-900 rounded-[2.5rem] p-8 text-white shadow-2xl space-y-6">
             <div>
                <label class="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-4 ml-1">Documento Ativo</label>
                @if($atleta->atl_documento)
                    <div class="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/10 mb-4">
                        <div class="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        </div>
                        <div class="flex-1 overflow-hidden">
                            <p class="text-[10px] font-black uppercase text-gray-400 truncate">Documento Enviado</p>
                            <a href="{{ asset('storage/doc_atletas/' . $atleta->atl_documento) }}" target="_blank" class="text-[9px] font-bold text-blue-400 uppercase tracking-widest">Visualizar Atual</a>
                        </div>
                    </div>
                @endif
                
                <div class="relative w-full">
                    <input type="file" name="atl_documento" id="atl_documento" accept=".pdf,image/*" class="absolute inset-0 opacity-0 z-10 cursor-pointer">
                    <div class="w-full bg-white/5 border border-dashed border-white/20 rounded-2xl p-6 flex flex-col items-center gap-2">
                        ...
                        <span class="text-[10px] font-bold text-gray-400 uppercase" id="filename-preview">Substituir Documento</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-3 pt-6 px-1">
            <button type="submit" class="w-full bg-blue-600 border border-blue-500 text-white font-extrabold py-5 rounded-2xl shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-3">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                Salvar Alterações
            </button>
            <a href="{{ route('atletas.index') }}" class="w-full text-gray-400 font-bold py-4 text-center">Descartar mudanças</a>
        </div>
    </form>
</div>
