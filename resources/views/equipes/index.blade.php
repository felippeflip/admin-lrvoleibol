<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="flex justify-start mb-4">
                        ...
                        <a href="{{ isset($time) ? route('equipes.create', ['time_id' => $time->tim_id]) : route('equipes.create') }}"
                            class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">NOVA
                            EQUIPE</a>
                    </div>

                    <div class="mb-6 bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow">
                        <form method="GET" action="{{ route('equipes.index') }}">
                            {{-- Se estiver filtrando por um time específico vindo de outra página, mantém o ID no form
                            hidden se não quiser mostrar o dropdown ou ajusta o dropdown --}}
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="search" id="search" value="{{ request('search') }}"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-800 dark:text-white placeholder-gray-400"
                                        placeholder="Ex: Sub-17, Série A...">
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="categoria" id="categoria"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-800 dark:text-white">
                                ...
                                <!-- Campeonato -->
                                <div>
                                    <label for="campeonato_id"
                                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">Campeonato</label>
                                ...
                                <div>
                                    ...
                                    <select name="time_id" id="time_id"
                                        ...
                                        <option value="">Todos</option>
                                        @foreach($times ?? [] as $t)
                                            <option value="{{ $t->tim_id }}" {{ (request('time_id') == $t->tim_id || (isset($time) && $time->tim_id == $t->tim_id)) ? 'selected' : '' }}>
                                                {{ $t->tim_nome }}
                                            </option>
                                        @endforeach
                                    </select>
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex flex-col gap-1">
                                                @forelse($equipe->campeonatos as $campeonato)
                                                    <span class="inline-flex items-center justify-between px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300 group">
                                                        {{ $campeonato->cpo_nome }}
                                                        
                                                        @role('Administrador')
                                                            <form action="{{ route('equipes.campeonato.destroy', ['campeonato' => $campeonato->cpo_id, 'equipe' => $equipe->eqp_id]) }}" 
                                                                  method="POST" 
                                                                  class="ml-2 inline-flex" 
                                                                  onsubmit="return confirm('Tem certeza que deseja remover esta equipe do campeonato {{ $campeonato->cpo_nome }}?');">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" class="text-blue-400 hover:text-red-500 focus:outline-none" title="Remover do Campeonato">
    ...
    <div id="campeonatosModal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <button type="button" onclick="closeCampeonatosModal()"
                ...
                <div class="px-6 py-6 lg:px-8">
                    <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Campeonatos de <span
                            id="modalEquipeNome" class="font-bold"></span></h3>
                    <ul id="modalCampeonatosList"
    ...
    <script>
        ...
        function openCampeonatosModal(nomeEquipe, campeonatos, equipeId) {
            ...
            if (campeonatos && campeonatos.length > 0) {
                ...
                campeonatos.forEach(camp => {
                    ...
                    li.className = "flex justify-between items-center bg-gray-50 dark:bg-gray-800 p-2 rounded";

                    const textSpan = document.createElement('span');
                    textSpan.textContent = `${camp.cpo_nome} (${new Date(camp.pivot.eqp_cpo_dt_inscricao).toLocaleDateString('pt-BR')})`;
                    li.appendChild(textSpan);

                    if (isAdmin) {
                        const form = document.createElement('form');
                        // Construct URL: /campeonatos/{campeonato}/equipes/{equipe}
                        ...
                        btn.type = 'submit';
                        btn.className = 'text-red-500 hover:text-red-700 ml-4';
                        btn.title = 'Remover do Campeonato';
                        btn.innerHTML = `
