                    ...
                    <div class="flex flex-col md:flex-row gap-8">
                        ...
                        <div class="w-full md:w-1/3 flex flex-col items-center">
                            <div class="w-48 h-64 border-4 border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden mb-4 shadow-lg">
                                <img src="{{ $user->foto_url }}" alt="Foto de {{ $user->name }}" class="w-full h-full object-cover">
                            </div>
                            <h3 class="text-2xl font-bold text-gray-800 dark:text-gray-100 text-center mb-1">{{ $user->name }}</h3>
                            ...
                            @if($user->hasRole('Juiz'))
                                <p class="text-lg text-gray-600 dark:text-gray-300 font-semibold mb-4 flex items-center">
                                    ...
                                    {{ $user->time->tim_nome ?? 'Sem Entidade Vinculada' }}
                                </p>
                            @endif
                             ...
                             @if($user->hasRole('ResponsavelTime'))
                                <p class="text-lg text-gray-600 dark:text-gray-300 font-semibold mb-4 flex items-center">
                                    Responsável pelo time: 
                                     @php
                                        // Fetch time manually if not in relation or need specific lookup
                                        $responsibleTime = \App\Models\Time::where('tim_user_id', $user->id)->first();
                                    @endphp
                                    {{ $responsibleTime->tim_nome ?? 'Nenhum Time' }}
                                </p>
                            @endif
                        </div>
                        ...
                        <div class="w-full md:w-2/3">
                            <div class="mb-8">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6">
                                    <div>
                                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Apelido</p>
                                        <p class="mt-1">{{ $user->apelido ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">CPF</p>
                                        <p class="mt-1">{{ $user->cpf ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">RG</p>
                                        <p class="mt-1">{{ $user->rg ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Data de Nascimento</p>
                                        <p class="mt-1">{{ $user->data_nascimento ? \Carbon\Carbon::parse($user->data_nascimento)->format('d/m/Y') : '-' }}</p>
                                    </div>
                                    @if($user->hasRole('Juiz'))
                                    <div>
                                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">CREF (Registro)</p>
                                        <p class="mt-1">{{ $user->cref ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Registro (LRV)</p>
                                        <p class="mt-1">{{ $user->lrv ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Categoria (Juiz)</p>
                                        <p class="mt-1">{{ $user->tipo_arbitro ?? '-' }}</p>
                                    </div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="mb-8">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6">
                                    <div>
                                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Telefone/Celular</p>
                            ...
                            <div class="flex justify-center gap-3 pt-6 border-t border-gray-200 dark:border-gray-700 no-print">
                                ...
                                <a href="{{ route('users.edit', $user->id) }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-md transition duration-150 ease-in-out flex items-center">
