                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ route('profiles.update', $profile) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" id="name" class="w-full border border-gray-300 p-2 rounded" value="{{ $profile->name }}" required>
                        </div>
                        <div class="mb-4">
                            ...
                            <select name="roles[]" id="roles" class="w-full border border-gray-300 p-2 rounded" multiple>
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}" {{ in_array($role->id, $profile->roles->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $role->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                            <label class="flex items-center">
                                <input type="checkbox" name="enable_mobile_view" value="1" {{ $profile->enable_mobile_view ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-blue-600">
